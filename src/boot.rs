...
pub fn fdt(l: &Layout, disk: bool, redist_size: u64, timers: [u32; 2]) -> Result<Vec<u8>> {
    ...
    f.end_node(n)?;
    if disk {
        let n = f.begin_node("virtio_mmio@a000000")?;
        f.property_string("compatible", "virtio,mmio")?;
        f.property_array_u64("reg", &[BLOCK, 0x1000])?;
        f.property_array_u32("interrupts", &[0, BLOCK_IRQ - 32, 4])?;
...
mod tests {
    ...
    fn layout_and_dtb() {
        ...
        let dtb = fdt(&l, true, 0x20000, [30, 27]).unwrap();
