...
默认仍为 1 核、512 MiB 基础内存、不启用动态内存。核数受宿主 HVF 上限约束。基础 RAM 不可移除；动态区域是额外内存，设备块大小由 `--virtio-mem-block-size-mib` 指定（默认 2，整数 MiB 二次幂，显式使用要求同时指定区域容量）。区域容量为 `max(128, block_size_mib)` 的正整数倍，区域起点按相同字节量对齐；目标初始为 0，须为设备块大小的整数倍且不超过区域容量，非法输入保留原目标。例如默认块大小下 6 MiB 合法、3 MiB 非法。状态 JSON 包含 `block_size_mib`。guest 实际调整粒度可能更大，目标接受不代表已完成调整；实际容量与目标可长期不同，不自动视为错误。地址范围受宿主 HVF IPA 位宽限制。
